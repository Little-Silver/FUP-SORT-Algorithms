...
--------------------------------------------------------------------------------------------
bsort4 :: Ord a => [a] -> [a]   
bsort4 list = bsort4' False [] list
    where 
        bsort4' :: Ord a => Bool -> [a] -> [a] -> [a]   
        bsort4' False s [x] = reverse (x:s)
        bsort4' True s [x]  = bsort4' False [] (reverse (x:s))
        bsort4' flag s (x:y:xs) | x > y      = bsort4' True (y:s) (x:xs)
                                | otherwise  = bsort4' flag (x:s) (y:xs)
